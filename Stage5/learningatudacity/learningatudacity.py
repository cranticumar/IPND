...
import logging
import time
from thirdpartysite_apis import *
...
# empty list objects for areas, userposts and courses at udacity
areas = []
userposts = []
courses = {}
...
# get courses of udacity through an API and add them to dictionary
# data is dumpted back into induvidual json objects so that it can be passed on to java script in the form of JSON string
courses = {course['title']: json_dump(course) for course in get_json_response('https://www.udacity.com/public-api/v0/courses')['courses']}
...
class Handler(webapp2.RequestHandler):
    ...
    def get_current_user(self):
        ...
        if self.request.cookies.get("user_id") and self.request.cookies.get("user_id") != '':
            return Users.get_by_username(self.request.cookies.get("user_id"))
        else:
            return None
    ...
    def read_params(self, **params):
        ...
        if self.__class__ not in [Blog] + Blog.__subclasses__():
            ...
            params['user_dispname'] = self.usr.dispname if self.usr else None
            params['templates'] = self.get_templates()
            params['courses'] = courses
            if not self.__class__ in [AddContent]:
                params['home'] = True
...
class AddContent(Handler):
    ...
    This page is accessible to only to specifc users (admins)
    """
    def get(self):
        """
        Provides a page to enter content and title fields to add content to website
        """
